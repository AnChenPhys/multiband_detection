...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel single-case TianQin Monte Carlo with FullPop mass and Madau redshift models'
    )
...
def worker_chunk(args):
    ...
    for i in range(n_samples):
        ...
        if window > 0.0:
            t_merger[i] = rng.uniform(0.1, 2000)
...
def save_detectable_samples(path, arrays, summary):
    ...
    header_lines = [
        'TianQin-detectable FullPop+Madau single-case samples with window_yr > 0',
        f'summary = {summary}',
...
def main():
    ...
    out_summary = args.out_summary or Path(__file__).with_name(f'tianqin_event_count_{tag}.txt')
    out_samples = args.out_samples or Path(__file__).with_name(f'tianqin_event_count_{tag}_samples.txt')
