...
REDSHIFT_GRID_SIZE = 3000


def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel single-case LISA Monte Carlo with FullPop mass and Madau redshift models'
    )
...
def lisa_time_window_bounds(m1, m2, z, observation_years, rho_thr):
...
def worker_chunk(args):
    ...
    for i in range(n_samples):
        window, lower, upper = lisa_time_window_bounds(m1[i], m2[i], z[i], t_obs_yr, rho_thr)
        windows[i] = window
...
def run_case(r0, t_obs_yr, rho_thr, z_max, n_mc, n_workers, seed_base):
    ...
    summary = {
        ...
        'n_workers': n_workers,
        'n_lisa_window_positive': int(np.count_nonzero(detectable)),
...
def save_detectable_samples(path, arrays, summary):
    ...
    header_lines = [
        'LISA-detectable FullPop+Madau single-case samples with window_yr > 0',
        f'summary = {summary}',
...
def main():
    ...
    out_summary = args.out_summary or Path(__file__).with_name(f'lisa_event_count_{tag}.txt')
    out_samples = args.out_samples or Path(__file__).with_name(f'lisa_event_count_{tag}_samples.txt')
