...
def ligo_psd_from_code_dir(f):
    data = np.loadtxt(Path(__file__).with_name('LIGO_O5_psd.txt'))
    return np.interp(f, data[:, 0], data[:, 1])


ligo_psd = ligo_psd_from_code_dir
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel FullPop+Madau LIGO event sampler using joint-polarization SNR threshold'
    )
...
def case_tag(rho_thr, z_max, seed):
    rho_tag = str(rho_thr).replace('.', 'p')
    z_tag = str(z_max).replace('.', 'p')
    s_tag = str(seed)
    return f'fullpop_madau_rho-{rho_tag}_zmax-{z_tag}_seed-{s_tag}'
...
def worker_chunk(args):
    ...
    for i in range(n_samples):
        snr[i], dl_mpc[i], chirp_mass[i] = joint_polarization_snr(
            m1[i],
            m2[i],
            z[i],
            cos_iota[i],
...
def run_case(rho_thr, z_max, n_mc, n_workers, n_freq, seed_base, mass_params):
    n_workers = max(1, n_workers)
    base = n_mc // n_workers
    remainder = n_mc % n_workers
    chunk_sizes = [base + (1 if i < remainder else 0) for i in range(n_workers)]
    cases = [
        (rho_thr, z_max, chunk_sizes[i], n_freq, seed_base + i, mass_params)
    ...
    arrays = {
        ...
        for name in ['m1', 'm2', 'chirp_mass', 'q', 'z', 'dl_mpc', 'cos_iota', 'snr', 'detected']
    }
...
def save_detected_samples(path, arrays, summary):
    ...
    header_lines = [
        'FullPop+Madau LIGO events with joint-polarization SNR above threshold',
        f'summary = {summary}',
        'columns: m1_msun m2_msun chirp_mass_msun q redshift dl_mpc cos_iota snr',
    ]
...
def main():
    ...
    tag = case_tag(args.rho_thr, args.z_max, args.seed)
    output_dir = Path(__file__).with_name('ligo_event_snr')
    out_summary = args.out_summary or output_dir / f'ligo_event_snr_{tag}_full.txt'
    out_samples = args.out_samples or output_dir / f'ligo_event_snr_{tag}_samples_full.txt'
