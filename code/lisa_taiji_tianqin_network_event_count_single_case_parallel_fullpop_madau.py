"""LISA/Taiji/TianQin network event count from combined SNR.
...
For a selected detector network, it computes each detector SNR curve on the same
merger-time grid and forms rho_net(t) = sqrt(sum_i rho_i(t)^2).

Examples:
    python lisa_taiji_tianqin_network_event_count_single_case_parallel_fullpop_madau.py --detector LISA-Taiji
    python lisa_taiji_tianqin_network_event_count_single_case_parallel_fullpop_madau.py --detector LISA-TianQin
    python lisa_taiji_tianqin_network_event_count_single_case_parallel_fullpop_madau.py --detector LISA-Taiji-TianQin
"""
...
RHO_THR = 8.0
Z_MAX = 1.0
T_MERGER_GRID_YR = np.logspace(-1, np.log10(2000), 200)
...
def parse_detector_network(detector):
    parts = [part.strip() for part in str(detector).replace(',', '-').split('-') if part.strip()]
    detectors = []
    for part in parts:
        key = part.lower()
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description='Parallel single-case LISA/Taiji/TianQin network Monte Carlo with combined SNR, FullPop masses, and Madau redshifts'
    )
    parser.add_argument('--detector', type=str, default=RUN_CONFIG['detector'],
                        help='Detector network, e.g. LISA-Taiji, LISA-TianQin, or LISA-Taiji-TianQin')
...
def case_tag(detectors, r0, t_obs_yr, rho_thr, z_max):
    ...
    return f'{det_tag}_fullpop_madau_R0-{r0_tag}_Tobs-{tobs_tag}_rho-{rho_tag}_zmax-{z_tag}'
...
def network_snr_vs_merger_time(m1, m2, luminosity_distance_mpc, merger_times_yr, detectors, observation_years):
...
def network_time_window_bounds(m1, m2, z, observation_years, rho_thr, detectors):
    dl_mpc = float(tnf.luminosity_distance_mpc_from_z(float(z)))
    _, rho_curve, _ = network_snr_vs_merger_time(
        m1,
        m2,
        dl_mpc,
        T_MERGER_GRID_YR,
...
def worker_chunk(args):
    detectors, t_obs_yr, rho_thr, z_max, n_samples, seed = args
    ...
    for i in range(n_samples):
        window, lower, upper = network_time_window_bounds(m1[i], m2[i], z[i], t_obs_yr, rho_thr, detectors)
        windows[i] = window
...
def run_case(detectors, r0, t_obs_yr, rho_thr, z_max, n_mc, n_workers, seed_base):
    ...
    cases = [
        (detectors, t_obs_yr, rho_thr, z_max, chunk_sizes[i], seed_base + i)
    ...
    summary = {
        'model': 'fullpop_madau',
        'detector_network': detector_tag(detectors),
        'detectors': detectors,
        'R0': r0,
        'Tobs_yr': t_obs_yr,
        'rho_thr': rho_thr,
        ...
        'n_workers': n_workers,
        'n_network_window_positive': int(np.count_nonzero(detectable)),
...
def save_detectable_samples(path, arrays, summary):
    ...
    header_lines = [
        f"{summary['detector_network']}-detectable FullPop+Madau network samples with combined-SNR window_yr > 0",
        f'summary = {summary}',
...
def output_paths(args, detectors):
    tag = case_tag(detectors, args.R0, args.Tobs, args.rho_thr, args.z_max)
    output_dir = module_path.parent
    out_summary = args.out_summary or output_dir / f'network_event_count_{tag}.txt'
    out_samples = args.out_samples or output_dir / f'network_event_count_{tag}_samples.txt'
...
def execute(args=None):
    ...
    out_summary, out_samples = output_paths(args, detectors)
