...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel FullPop+Madau TianQin event sampler using joint-polarization SNR threshold'
    )
...
def joint_polarization_snr(m1, m2, z, t_merger_yr, cos_iota, observation_years, n_freq):
    ...
    psd = tnf.tianqin_psd(frequencies, observation_years=observation_years)
...
def save_detected_samples(path, arrays, summary):
    ...
    header_lines = [
        'FullPop+Madau TianQin events with joint-polarization SNR above threshold',
        f'summary = {summary}',
...
def main():
    ...
    out_summary = args.out_summary or f'tianqin_event_snr/tianqin_event_snr_{tag}.txt'
    out_samples = args.out_samples or f'tianqin_event_snr/tianqin_event_snr_{tag}_samples.txt'
