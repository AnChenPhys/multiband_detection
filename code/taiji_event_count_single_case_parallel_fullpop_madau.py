...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel single-case Taiji Monte Carlo with FullPop mass and Madau redshift models'
    )
...
def save_detectable_samples(path, arrays, summary):
    ...
    header_lines = [
        'Taiji-detectable FullPop+Madau single-case samples with window_yr > 0',
        f'summary = {summary}',
...
def main():
    ...
    out_summary = args.out_summary or Path(__file__).with_name(f'taiji_event_count_{tag}.txt')
    out_samples = args.out_samples or Path(__file__).with_name(f'taiji_event_count_{tag}_samples.txt')
