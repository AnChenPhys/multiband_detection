...
RHO_THR = 8.0
Z_MAX = 1
T_MERGER_GRID_YR = np.logspace(-3, np.log10(20), 200)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Parallel single-case AMIGO Monte Carlo with FullPop mass and Madau redshift models'
    )
    parser.add_argument('--R0', type=float, default=R0)
    parser.add_argument('--LAMIGO', type=float, default=LAMIGO)
...
def case_tag(r0, t_obs_yr, rho_thr, z_max, lamigo):
    r0_tag = str(r0).replace('.', 'p')
    tobs_tag = str(t_obs_yr).replace('.', 'p')
    rho_tag = str(rho_thr).replace('.', 'p')
    z_tag = str(z_max).replace('.', 'p')
    lamigo_tag = str(lamigo/1e7)
    return f'fullpop_madau_R0-{r0_tag}_Tobs-{tobs_tag}_rho-{rho_tag}_zmax-{z_tag}_LAMIGO-{lamigo_tag}'
...
def amigo_time_window_bounds(m1, m2, z, observation_years, rho_thr, LAMIGO):
    dl_mpc = float(tnf.luminosity_distance_mpc_from_z(float(z)))
    _, rho_curve = tnf.snr_vs_merger_time(
        m1,
        m2,
        dl_mpc,
        T_MERGER_GRID_YR,
    ...
    if not crossings:
        if rho_curve[0] < rho_thr:
            return 0.0, np.nan, np.nan
        lower = T_MERGER_GRID_YR[0]
        upper = T_MERGER_GRID_YR[-1]
...
def worker_chunk(args):
    t_obs_yr, rho_thr, z_max, lamigo, n_samples, seed = args
    ...
    for i in range(n_samples):
        window, lower, upper = amigo_time_window_bounds(m1[i], m2[i], z[i], t_obs_yr, rho_thr, LAMIGO=lamigo)
        windows[i] = window
...
def run_case(r0, t_obs_yr, rho_thr, z_max, lamigo, n_mc, n_workers, seed_base):
    n_workers = max(1, n_workers)
    base = n_mc // n_workers
    remainder = n_mc % n_workers
    chunk_sizes = [base + (1 if i < remainder else 0) for i in range(n_workers)]
    cases = [
        (t_obs_yr, rho_thr, z_max, lamigo, chunk_sizes[i], seed_base + i)
    ...
    summary = {
        ...
        'n_workers': n_workers,
        'n_amigo_window_positive': int(np.count_nonzero(detectable)),
...
def save_detectable_samples(path, arrays, summary):
    ...
    header_lines = [
        'AMIGO-detectable FullPop+Madau single-case samples with window_yr > 0',
        f'summary = {summary}',
...
def main():
    ...
    tag = case_tag(args.R0, args.Tobs, args.rho_thr, args.z_max, args.LAMIGO)
    out_summary = args.out_summary or Path(__file__).with_name(f'amigo_event_count_{tag}.txt')
    out_samples = args.out_samples or Path(__file__).with_name(f'amigo_event_count_{tag}_samples.txt')
